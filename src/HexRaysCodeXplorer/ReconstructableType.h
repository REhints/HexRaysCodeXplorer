...
public :
	...
	MemberTypePointer(const std::string &t);
...
public:
	...
	virtual bool SetMemberType(unsigned long offset, ReconstructedMemberType* newType);
	virtual bool AddMember(ReconstructableMember* member);
	ea_t to_ea(unsigned long offset);

	static ReconstructableTypeVtable * getReconstructableTypeVtable(const std::string &Name, ea_t addr);
};
