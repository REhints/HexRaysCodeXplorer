...
static bool idaapi reconstructed_types_dblclick(TWidget *v, int shift, void *ud)
{
	...
	case REPLACE_MEMBERS:
		if (replace->atOwnMembers == false) {
			std::string typeName = derivedMembers[replace->own_offset]->memberType->getTypeString();
			if (g_ReconstractedTypes.count(typeName)) {
				place_t *plce = replace->makeplace(0, g_ReconstractedTypes[typeName]->typeId, g_ReconstractedTypes[typeName]->typeId);
...
static void idaapi reconstructed_adjust_place(TWidget *v, lochist_entry_t *loc, void *ud) {
	reconstructed_place_t *re_place = (reconstructed_place_t*)(loc->plce);
	int x, y;
	place_t *place = get_custom_viewer_place(v, true, &x, &y);
	reconstructed_place_t *replace = (reconstructed_place_t *)place;
}
...
ssize_t hook_idb_events(void *user_data, int notification_code, va_list va) {
	...
	case idb_event::expanding_struc:
		...
		reType = g_ReconstractedTypes[oldname];
		if (reType->maxSize < ea + diff)
	...
	case idb_event::deleting_struc_member:
		...
		if (g_ReconstractedTypes.count(oldname) == 0)
			break;
		reType = g_ReconstractedTypes[oldname];
		reType->UndefMembers(member->soff, member->eoff - member->soff, true);
		break;
	...
	case idb_event::renaming_struc_member:
		...
		if (g_ReconstractedTypes.count(oldname) == 0)
			break;
		reType = g_ReconstractedTypes[oldname];
		reType->SetMemberName(member->soff, newname);
		break;
	...
	case idb_event::struc_member_changed:
		...
		members = reType->getOwnMembers();
		if (members.count(member->soff) == 0)
			assert(false);
		reMember = members[member->soff];
		size = tinfo.get_size();
		if (reMember->getSize() < size) {
			reType->UndefMembers(reMember->getSize() + reMember->offset, size - reMember->getSize(), true);
		}
		...
		if (g_ReconstractedTypes.count(q_string.c_str()))
			...
			reMemberType = new MemberTypeIDATypeInfoGate(tinfo);
		reType->SetMemberType(member->soff, reMemberType);
		break;
...
void re_types_form_init()
{
	...
	*/
	...
	reconstructed_types_info_t *si = new reconstructed_types_info_t(widget);
	if (g_ReconstractedTypes.size()) {
		std::string typeName = g_ReconstractedTypes.begin()->first; // bug if no any types found 
		reconstructed_place_t start(typeName.c_str());
		typeName = g_ReconstractedTypes.rbegin()->first;
		reconstructed_place_t end(typeName.c_str());
		...
		si->cv = create_custom_viewer("", (place_t*)&start, (place_t*)&end, &start, nullptr, nullptr, nullptr, nullptr, widget);
	}
	else {
		reconstructed_place_t start;
		si->cv = create_custom_viewer("", (place_t*)&start, (place_t*)&start, &start, nullptr, nullptr, nullptr, nullptr, widget);
	}
...
{
	ReconstructedMemberReType * result = new ReconstructedMemberReType(*this);
	return result;
}
...
MemberTypePointer::MemberTypePointer(const std::string &t) :
	ReconstructedMemberType(MemberType_Pointer), pointedType(t)
{
}
...
{
	ea_t func_addr = getEa(vtable_address + sizeof(ea_t) * 2 + sizeof(ea_t) * offset);
	std::string funcname = newName;
	std::string class_name = name.substr(0, name.length() - sizeof(VTBL_CLSNAME_POSTFIX));
	// name.substr(0, name.length() - sizeof(VTBL_CLSNAME_POSTFIX))
	ReconstructableMember *member = findMemberByOffset(offset, false);
	if (!member)
		return false;
	char *ptr;
	if (ptr = std::strstr((char *)newName, class_name.c_str())) {
		ptr += class_name.size();
		if (ptr[0] == ':' && ptr[1] == ':')
		{
			ptr += 2;
	...
	if (func) {
		std::string long_name = class_name;
		long_name += "::";
		long_name += funcname;
...
bool ReconstructableTypeVtable::SetMemberType(unsigned long offset, ReconstructedMemberType * newType)
{
	return ReconstructableType::SetMemberType(offset, newType);
}
...
ea_t ReconstructableTypeVtable::to_ea(unsigned long offset)
{
	ea_t res;
	res = getEa(vtable_address + sizeof(ea_t) * 2 + offset);

	return res;
}

ReconstructableTypeVtable * ReconstructableTypeVtable::getReconstructableTypeVtable(const std::string & Name, ea_t addr)
{
	ReconstructableTypeVtable *reType = 0;
	if (g_ReconstractedTypes.count(Name))
		return dynamic_cast<ReconstructableTypeVtable *>(g_ReconstractedTypes[Name]);
	reType = new ReconstructableTypeVtable(Name, addr);
	g_ReconstractedTypes[Name] = reType;
	return reType;
}
